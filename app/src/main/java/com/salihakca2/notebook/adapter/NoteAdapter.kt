...
class NoteAdapter(val noteList: List<Note>): RecyclerView.Adapter<NoteAdapter.NoteHolder>() {
    ...
    override fun onBindViewHolder(holder: NoteHolder, position: Int) {
        holder.recyclerRowBinding.textView.text = noteList.get(position).baslik
        holder.itemView.setOnClickListener {
            val intent = Intent(holder.itemView.context, AddNoteActivity::class.java)
            intent.putExtra("NotePlace", noteList.get(position)) //position aldık
            intent.putExtra("info","old") //info yolladık
